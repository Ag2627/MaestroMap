...
export default function GooglePlaces() {
  ...
  const handleSearch = async () => {
    ...
    const bodyData = {
      ...
      types: GOOGLE_TYPES.join(","), // send all types
      limit: 100,
    };
    ...
    try {
      ...
      const formatted = data
        ...
        .filter(Boolean)
        .sort((a, b) => (b.reviews || 0) - (a.reviews || 0)); // most reviewed first
        ...
        <div>
          ...
          {places.map((p, i) => (
            ...
            >
              ...
              {p.photo && <img src={p.photo} alt={p.name} className="mt-2 w-full rounded-md border" />}
            </div>
          ))}
        </div>
      </div>
  ...
  >
    <div className="max-w-xs">
      ...
      <div className="text-sm text-yellow-600">
        ⭐ {selectedPlace.rating || "N/A"} ({selectedPlace.reviews || 0} reviews)
      </div>
      {selectedPlace.photo && (
        <img src={selectedPlace.photo} alt={selectedPlace.name} className="mt-2 w-full rounded-md" />
      )}
      {/* View in Maps button */}
      <a
