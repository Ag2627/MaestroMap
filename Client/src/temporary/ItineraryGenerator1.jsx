...
export default function ItineraryGenerator1() {
  ...
  const handlePlanTrip = async () => {
    ...
    const planPromise = Promise.all([
      ...
      }).then(res => { if(!res.ok) throw new Error("Failed to generate itinerary."); return res.json(); }),
        ...
        <div className="bg-white/90 backdrop-blur-sm border border-orange-200 shadow-lg rounded-2xl p-8 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-6 items-end">
            <div className="md:col-span-3 space-y-2">
              <label className="font-medium text-gray-700 flex items-center gap-2"><FaMapMarkerAlt className="text-orange-500"/> Destination</label>
              <Autocomplete onLoad={auto => destAutoRef.current=auto}>
                <input type="text" placeholder="Enter a city, e.g., Rome, Italy" className="w-full p-3 border border-orange-300 rounded-lg focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition"/>
              </Autocomplete>
            </div>
            <div className="md:col-span-1 space-y-2">
              <label className="font-medium text-gray-700 flex items-center gap-2"><FaCalendarAlt className="text-orange-500"/> Days</label>
              <select value={days} onChange={e=>setDays(Number(e.target.value))} className="w-full p-3 border border-orange-300 rounded-lg focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition">
